...
$(document).ready(function () {
    ...
    //$("#loadPageMenu").load('~/../../../../../../../../../../../../static/page_menu/menu_contabilidad.aspx');
...
function fnLoadPageInDialog(url, contID, titleNewDialog, widthNewDialog, heightNewDialog) {
    ...
    //var $dialog = $("#dialogWindows")
    var $dialog = $('<div style="overflow: scroll" id="dialogWindows' + number + '"><div class="body_change' + number + '" ></div></div>')
          .css({ overflow: "scroll", background: "white", padding: "0", margin: "0 0 -25px 0" })
          //.html('<object type="text/html" data="' + url + '" width="100%" height="100%" style:"border: 0;"></object>')
          .dialog({
              iconButtons: [
                   {
                       text: "Restaurar",
              ...
              dialogClass: 'color_close myTitleDialog  ui-dialog-osx'
          });

    $(".body_change"+number).load(url);
    $dialog.dialog('open');
...
function fnLoadPageInDialog2(url, contID, titleNewDialog, widthNewDialog, heightNewDialog) {
    var urlWithCont = url;
    if (contID != '')
        urlWithCont = urlWithCont + ' #' + contID;
    ...
    var $dialog = $('<div></div>')
          .css({ overflow: "auto", background: "#ccc9c9", padding: "0" })
          .html('<object type="text/html" data="' + url + '" width="100%" height="' + (heightNewDialog) + '" style:"border: 0;"></object>')
          .dialog({
              iconButtons: [
                    /*{
                        text: "Add",
                        ...
                        click: function (e) {
                            $(this).parents('.ui-dialog').animate({
                                ...
                                top: ($(window).height() - heightNewDialog) / 2,
              ...
              dialogClass: 'myTitleDialog ui-dialog-osx'
          });
...
function accessLink(id, array_data) {

    LoadDivis();
    
    $.ajax({
        type: "POST",
        url: "index.aspx/PermissionLink",
        data: "{idmenu: '" + id + "'}",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (res) {
            ...
            if (sect == false) {
                ViewInfo('No presenta acceso a este modulo, contacte con el Administrador del Sistema...');
            } else {
                if (array_data.length == 1) {
                    window.open(array_data[0]);
                } else {
                    fnLoadPageInDialog2(array_data[0], array_data[1], array_data[2], array_data[3], array_data[4]);
                }
            }
        },
        error: function () { }
